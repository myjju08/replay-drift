#!/usr/bin/env bash
# Four concurrent full-feature, two-GPU reverse-kernel bandwidth ablations.
set -euo pipefail
...
for variant in "${VARIANTS[@]}"; do
  workdir="${ROOT}/runs/gen_B4_revdrift_mae256_full86_p128_n32_kerneltemp_${variant}_${ABLATION_TAG}"
...
for index in "${!VARIANTS[@]}"; do
  ...
  echo "[kernel-temperature] launching ${variant} GPUs=${GPU_GROUPS[$index]} log=${log_file}"
  setsid -f env \
    OMP_NUM_THREADS=1 \
    MKL_NUM_THREADS=1 \
    ...
    "${TORCHRUN_BIN}" \
      ...
      "${ROOT}/scripts/run_B4_reverse_kernel_temperature_ablation.py" \
      --config "${CONFIG}" \
      --workdir "${workdir}" \
      --variant "${variant}" \
      --seed "${TRAIN_SEED}" \
  ...
  sleep 2
  pid="$(pgrep -f "run_B4_reverse_kernel_temperature_ablation.py.*--workdir ${workdir}.*--variant ${variant}" | head -n 1 || true)"
  if [[ -n "${pid}" ]]; then
    echo "${pid}" > "${pid_file}"
    echo "[kernel-temperature] ${variant} pid=${pid}"
  else
    echo "[warn] ${variant} launched but PID auto-detection failed"
  fi
done

echo "[kernel-temperature] all four runs launched through ${GENERATED_EPOCHS} generated epochs"
for variant in "${VARIANTS[@]}"; do
  echo "tail -f ${LOG_DIR}/gen_B4_revdrift_mae256_full86_p128_n32_kerneltemp_${variant}_${ABLATION_TAG}.log"
done
