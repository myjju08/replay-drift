#!/usr/bin/env bash
# L/2 baseline + MAE-640 + n_cls_tokens=16 — GPU 0..7 (8x RTX 5090)
#   pos_per_sample=16, neg_per_sample=16, gen_per_label=16
#   batch_size=4, total_steps=250228, ckpt every 25022 steps
#   wandb project: "ImageNet - L2"
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

CONFIG="${ROOT}/configs/gen/L2_rev-drift.yaml"
WORKDIR="${WORKDIR:-${ROOT}/runs/gen_L2_baseline_mae640_ncls8_gpu01234567_posneg16}"
LOG_DIR="${ROOT}/runs/launch_logs"
LOG_FILE="${LOG_DIR}/L2_baseline_mae640_ncls8_gpu01234567.log"
PID_FILE="${LOG_DIR}/pid_L2_baseline_mae640_ncls8_gpu01234567"
...
echo "[launch] L/2 baseline (8 GPU, pos/neg/gen=16, mae640, bs=4)"
