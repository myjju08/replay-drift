#!/usr/bin/env bash
# L/2 + MAE-640 + mixed_baseline_versionb + ADAPTIVE hedge (preset 3 = responsive, η×5)
#   gamma=2.0, eta=5e-4, decay=1e-4, gamma_warmup_steps=125114 (= T/2)
#   stronger data response: vb_coef ≈ 0.094 at step 200k vs 0.174 for g2.
#   8x RTX 5090, GPU 0..7. wandb project: "ImageNet - L2".
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

CONFIG="${ROOT}/configs/gen/L2_dual-drift_hedge_g2_e5x.yaml"
WORKDIR="${WORKDIR:-${ROOT}/runs/gen_L2_mix_adaptive_hedge_g2_e5x_mae640_ncls8_gpu01234567_neg32}"
LOG_DIR="${ROOT}/runs/launch_logs"
LOG_FILE="${LOG_DIR}/L2_mix_adaptive_hedge_g2_e5x_mae640_ncls8_gpu01234567_neg32.log"
PID_FILE="${LOG_DIR}/pid_L2_mix_adaptive_hedge_g2_e5x_mae640_ncls8_gpu01234567_neg32"
